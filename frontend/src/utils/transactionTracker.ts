 ...
 *
 * After executeTransaction() returns a txId, the transaction has been
 * broadcast but not yet confirmed. The finalize phase runs on-chain and
 * can REJECT the TX (e.g. failed assertions). This module polls the
 * explorer API until the TX is confirmed (accepted or rejected) or a
 * timeout is reached.
 *
 ...
 *
 * Shield Wallet specifics:
 *   executeTransaction() returns a TEMPORARY "shield_…" ID.
 *   We must poll wallet.transactionStatus(shieldId) first to resolve it
 *   to the real on-chain "at1…" TX ID before polling the explorer.
 */
...
export interface TxConfirmation {
  ...
  /** Real on-chain at1… ID (resolved from shield_ temp ID if needed). */
  txId: string;
...
function interpretResponse(txId: string, data: any): TxConfirmation {
  ...
  if (data?.type === 'rejected' || data?.status === 'rejected') {
    return { status: 'rejected', txId, rejectionReason: extractRejectionReason(data) };
  }
...
function extractRejectionReason(data: any): string {
  ...
  return (
    ...
    'Common causes: insufficient credits, wrong input format, or program logic assertion failed.'
  );
}
 ...
 * Poll wallet.transactionStatus until it provides the real on-chain "at1…" TX ID.
 ...
 *
 * @param txId            Transaction ID — either a real "at1…" or Shield Wallet "shield_…" temp ID
 ...
 * @param walletGetStatus Optional wallet.transactionStatus function.
 *                        REQUIRED when txId is a "shield_…" temp ID.
 */
export async function trackTransaction(
  ...
  if (txId.startsWith('shield_')) {
    if (!walletGetStatus) {
      console.warn('[TxTracker] shield_ ID received but walletGetStatus not provided. Assuming accepted.');
      return { status: 'accepted', txId };
    }
  ...
  // Initial 5 s delay — give the network time to include the TX in a block
  ...
  while (Date.now() - start < maxWaitMs) {
    ...
    if (result.status === 'rejected') {
      onProgress?.('Transaction REJECTED on-chain');
      return { ...result, txId: realTxId };
    }
